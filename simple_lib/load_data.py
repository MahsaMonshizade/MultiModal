...
def load_data_mean(path,cancer_type):
    ...
    meta_data=pd.read_csv(os.path.join(path, "filtered_metadata.csv"),index_col=0)
    data_class={'nonIBD':0,'CD':1,'UC':1}
    meta_data['diagnosis']=meta_data['diagnosis'].map(data_class)
    metagenomics = pd.read_csv(os.path.join(path, "filtered_metagenomics.csv"), index_col=0)
    metatranscriptomics = pd.read_csv(os.path.join(path, "filtered_metatranscriptomics.csv"), index_col=0)
    metabolomics = pd.read_csv(os.path.join(path, "filtered_metabolomics.csv"), index_col=0)
